...
$router->get('/api/me', fn($request)=>$authController->me($request), [fn($request)=>(bool)$authMiddleware->handle($request)]);
...
$router->post('/api/playback-sessions', fn()=>Response::json(['error'=>'Playback policy + signed stream session not implemented yet'],501), [fn($request)=>(bool)$authMiddleware->handle($request)]);
